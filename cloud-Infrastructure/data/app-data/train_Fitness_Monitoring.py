...
missing_value_describe(fitness_data)

fitness_data = fitness_data.drop(['date'], axis=1)
fitness_data = fitness_data.drop(['time'], axis=1)
fitness_data.columns

# dimension
print("the dimension:", fitness_data.shape)
...
def test_model(name, model):
    ...
    # Export model
    joblib.dump(model, gzip.open('model/fitness-' + name + '.dat.gz', "wb"))
